...
def history_chatter(prompt):
    ...
    refined_answer = agent2(answers_from_agent1)
    agent3 = DialogAgent(name="final_answer",
                         model_config_name="qwen_1.5_72B",
                         sys_prompt="帮我整理问题和答案，输出最终的答案给我，不用说之前的答案正确与否。"
                                    "如果回答有引用的请在最后用《书名》[原文]格式展示出来"
                         )
    final_prompt = f"原问题:{prompt}"+ f"答案{refined_answer['content']}"
    final_answer = agent3(final_prompt)
